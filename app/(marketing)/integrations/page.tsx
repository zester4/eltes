...
const DOMAINS = [
  ...
  {
    category: "Finance",
    tools: ["Stripe", "Wise", "PayPal", "QuickBooks", "Xero", "FreshBooks"],
  },
  {
    category: "Project & Code",
    tools: ["GitHub", "GitLab", "Jira", "Linear", "Asana", "ClickUp", "Notion"],
  },
  {
    category: "Product & Analytics",
    tools: ["Amplitude", "Mixpanel", "Segment", "PostHog"],
  },
  {
  ...
  },
  {
    ...
    tools: ["Mailchimp", "ConvertKit", "Klaviyo", "Twitter/X", "LinkedIn"],
  },
  { category: "Legal & Contracts", tools: ["DocuSign", "PandaDoc"] },
  {
    category: "Scheduling & Automation",
    tools: ["QStash", "Upstash Workflow", "Calendly"],
  },
    ...
    <div className="w-full relative overflow-visible bg-black selection:bg-primary/20">
      ...
      <section className="relative w-full min-h-[300px] md:min-h-[500px] py-12 md:py-32 px-4 sm:px-6 flex flex-col items-center justify-center text-center overflow-hidden border-b border-white/5">
        ...
        <div className="relative z-10 max-w-4xl mt-6 lg:mt-0">
          ...
          <p className="font-body font-light text-white/60 text-sm md:text-lg max-w-xl mx-auto leading-relaxed mb-6 md:mb-10 px-2">
            1,000+ integrations. Your agents don't need you to migrate to a new
            tool — they come to yours.
          </p>
        </div>
      </section>
      ...
      <section className="py-10 md:py-20 px-4 sm:px-6 md:px-16 lg:px-24 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-16 md:mb-24">
          {DOMAINS.map((domain) => (
            <div
              className="liquid-glass rounded-2xl md:rounded-[32px] p-5 md:p-8 border border-white/5 shadow-xl md:shadow-2xl"
              key={domain.category}
            >
              <h3 className="text-lg md:text-xl font-heading italic text-white mb-4 md:mb-6 tracking-tight">
                {domain.category}
              </h3>
              <ul className="flex flex-col gap-2 md:gap-3">
                {domain.tools.map((tool) => (
                  <li
                    className="text-white/60 font-body text-xs md:text-sm flex items-center gap-2.5"
                    key={tool}
                  >
                    <Globe className="w-3 h-3 md:w-4 md:h-4 text-white/20" />{" "}
                    {tool}
                  </li>
                ))}
              </ul>
        ...
        <div className="mt-10 md:mt-16 text-center max-w-3xl mx-auto liquid-glass p-8 md:p-12 rounded-3xl md:rounded-[40px] border border-white/5">
          <h3 className="text-xl md:text-3xl font-heading italic text-white mb-3 md:mb-6">
            Don't see your tool?
          </h3>
          <p className="font-body text-white/60 mb-6 md:mb-8 text-sm md:text-base">
            Etles uses Composio's universal connector layer. If it has an API,
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div>
              <h2 className="text-2xl md:text-4xl font-heading italic text-white mb-4 md:mb-6">
                Managed OAuth 2.0 Security
              </h2>
              <p className="font-body font-light text-white/50 text-sm md:text-base leading-relaxed mb-6 md:mb-8">
                Every connection is isolated and secured. We handle the complex
                handshake, refreshing, and token storage so you can focus on
  ...
  "security": "AES-256-GCM"
      ...
      <section className="py-12 md:py-24 px-4 sm:px-6 flex flex-col items-center justify-center border-t border-white/10 mt-12 md:mt-24">
        <p className="text-sm md:text-xl font-body font-light text-white/60 mb-6 md:mb-10 text-center max-w-2xl">
          Etles doesn't replace your stack. It becomes the intelligence layer
          that sits above it and operates it on your behalf.
        </p>
        <h2 className="text-2xl md:text-5xl font-heading italic text-white mb-6 md:mb-8 text-center uppercase tracking-tight">
          Hook it up.
        </h2>
        <Link
